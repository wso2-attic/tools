...
var _AARValidate = false;
var _ServicesXmlValidat = false;
...
function extraStoreDirUUID(uuid) {
    ...
    if (_AARValidate) {
        ...
        var bodyXml = '<soapenv:Body>\n' +
                      '<req:validateRequest xmlns:req="http://aar.tools.wso2.org/xsd">\n' +
                      '<fileId>' + uuid + '</fileId>\n' +
                      '</req:validateRequest>\n' +
                      '</soapenv:Body>\n';

        var serviceURL = _mainURL + "/services/" + "AarValidatorXSL";

        serviceClient(serviceURL, bodyXml, "validate");
    }

    if (_ServicesXmlValidat) {
        // do module Validation
        _ServicesXmlValidat = false;
        var bodyXml = '<soapenv:Body>\n' +
                      '<req:validateModuleXMLRequest xmlns:req="http://aar.tools.wso2.org/xsd">\n' +
                      '<fileId>' + uuid + '</fileId>\n' +
...
function initURL() {
    ...
    URL = _mainURL + "/wservices/AarValidatorXSL";
}
...
function validateAndSubmitServicesXml() {
    _ServicesXmlValidat = true;
    ...
    if (inputObj.value == "") {
        alert('Please Select a services.xml to Validate');
        return false;
    }
    formObj.action = _mainURL + "/fileupload/*";
    formObj.submit();
