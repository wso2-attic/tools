...
function extraStoreDirUUID(uuid) {
    ...
    var bodyXml = '<soapenv:Body>\n' +
                  '<req:convertRequest xmlns:req="http://org.wso2.tools.wsdl/xsd">\n' +
                  '<fileId>' + uuid + '</fileId>\n' +
                  '</req:convertRequest>\n' +
                  '</soapenv:Body>\n';

    var serviceURL = _mainURL + "/services/" + "WSDLConverter";

    serviceClient(serviceURL, bodyXml, "convert")
...
function initURL() {
    ...
    URL = _mainURL + "/wservices/WSDLConverter";
}
    ...
    req.onreadystatechange = function() {
        ...
        if (req.readyState == 4) {
            ...
            if (req.status == 200) {
                ...
                _response = req.responseXML;
                var _returnValue = _response.getElementsByTagName('return')[0].firstChild.nodeValue;
                window.location = _mainURL + _returnValue;
            } else {
//                alert("There was a problem retrieving the XML data:\n" + req.statusText);
                var exceptionNode = getExceptionNode(req.responseXML);
                alert("WODEN Encounters an Error when Converting\n"+xmlSerializerToString(exceptionNode));
            }
        }
