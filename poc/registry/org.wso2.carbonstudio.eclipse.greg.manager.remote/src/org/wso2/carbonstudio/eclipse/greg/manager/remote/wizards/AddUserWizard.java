...
public class AddUserWizard extends Wizard {
	...
	public boolean performFinish() {
		try {
			...
			exHandler.showMessage(Display.getCurrent().getActiveShell(),
					"Duplicate user name. Please enter another user name");
			return false;
		}
	...
	public void doFinish() throws RemoteException, UserAdminExceptionException,
		...
		if (isNew) {
			...
			regUserManager.changePWD(regUser.getUserName(), userInfoPage
					.getPwd());
			regUserManager
					.updateUserRoles(regUser.getUserName(), selectedRoles);
		}
	}
