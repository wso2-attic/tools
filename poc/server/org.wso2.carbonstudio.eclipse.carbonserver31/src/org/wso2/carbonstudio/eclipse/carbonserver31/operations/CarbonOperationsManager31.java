...
package org.wso2.carbonstudio.eclipse.carbonserver31.operations;
...
import org.wso2.carbonstudio.eclipse.carbonserver31.monitor.CarbonServerListener;
import org.wso2.carbonstudio.eclipse.carbonserver31.util.CarbonServer31Utils;
import org.wso2.carbonstudio.eclipse.carbonserver31.util.CarbonServerConstants;
...
//					break;
				...
				case ICarbonOperationManager.OPERATION_GET_SERVER_PORTS:
					if (server!=null)
						result=CarbonServer31Utils.getServerPorts(server);
					break;
				...
				case ICarbonOperationManager.OPERATION_GET_SERVER_CREDENTIALS:
					...
					if (server!=null){
						serverCredentials=CarbonServer31Utils.getServerCredentials(server);
					}
				...
				case ICarbonOperationManager.OPERATION_GET_SERVER_AUTHENTICATED_COOKIE:
					...
					if (server!=null){
						if (operation.containsKey(ICarbonOperationManager.PARAMETER_SERVER_PORT)){
							...
							cookie=CarbonServer31Utils.getServerCookie(server,serverPort);
						}
					}
					result=cookie;
					break;
...
//		String[] paths=new String[]{"repository","deployment","server"};
		String serverLocalRepoLocation=CarbonServer31Utils.getRepositoryPathFromLocalWorkspaceRepo(serverLocalWorkspacePath);
//		paths=new String[]{"repository","conf"};
		String serverLocalConfLocation=CarbonServer31Utils.getConfPathFromLocalWorkspaceRepo(serverLocalWorkspacePath);
...
//		CarbonServer31Utils.updateAndSaveCarbonXml(FileUtils.addNodesToPath(serverLocalConfLocation,new String[]{"carbon.xml"}), serverLocalRepoLocation,server);
		CarbonServer31Utils.updateTransportXML(server);
		CarbonServer31Utils.updateAxis2XML(server);
		//just retrieving the value for hotupdate will update the axis2.xml also
		CarbonServer31Utils.isServerHotUpdate(server);
