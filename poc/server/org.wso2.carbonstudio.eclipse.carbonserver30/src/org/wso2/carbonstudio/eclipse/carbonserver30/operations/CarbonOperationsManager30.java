...
package org.wso2.carbonstudio.eclipse.carbonserver30.operations;
...
import org.wso2.carbonstudio.eclipse.carbonserver.base.exception.CarbonServerNotRunningException;
...
import org.wso2.carbonstudio.eclipse.carbonserver30.monitor.CarbonServerListener;
import org.wso2.carbonstudio.eclipse.carbonserver30.util.CarbonServer30Utils;
import org.wso2.carbonstudio.eclipse.carbonserver30.util.CarbonServerConstants;
...
public class CarbonOperationsManager30 implements ICarbonOperationManager {
	...
	private static final String runtimeId="org.wso2.carbonstudio.eclipse.carbon.runtime30"; 
...
//					break;
				...
				case ICarbonOperationManager.OPERATION_GET_SERVER_PORTS:
					if (server!=null)
						result=CarbonServer30Utils.getServerPorts(server);
					break;
				...
				case ICarbonOperationManager.OPERATION_GET_SERVER_CREDENTIALS:
					...
					if (server!=null){
						serverCredentials=CarbonServer30Utils.getServerCredentials(server);
					}
				...
				case ICarbonOperationManager.OPERATION_GET_SERVER_AUTHENTICATED_COOKIE:
					...
					if (server!=null){
						if (operation.containsKey(ICarbonOperationManager.PARAMETER_SERVER_PORT)){
							...
							cookie=CarbonServer30Utils.getServerCookie(server,serverPort);
						}
					}
					result=cookie;
					break;
...
//		String[] paths=new String[]{"repository","deployment","server"};
		String serverLocalRepoLocation=CarbonServer30Utils.getRepositoryPathFromLocalWorkspaceRepo(serverLocalWorkspacePath);
//		paths=new String[]{"repository","conf"};
		String serverLocalConfLocation=CarbonServer30Utils.getConfPathFromLocalWorkspaceRepo(serverLocalWorkspacePath);
...
//		}
//		CarbonServer30Utils.updateAndSaveCarbonXml(FileUtils.addNodesToPath(serverLocalConfLocation,new String[]{"carbon.xml"}), serverLocalRepoLocation,server);
		CarbonServer30Utils.updateTransportXML(server);
		CarbonServer30Utils.updateAxis2XML(server);
		//just retrieving the value for hotupdate will update the axis2.xml also
		CarbonServer30Utils.isServerHotUpdate(server);
...
//		fileUtils.deleteFolder(serverLocalWorkspacePath);
	...
	private void initializeServerRepoLibraries(String wsasHomeFodler) throws CoreException{
		...
		if (!(new File(wsasHome.append("repository").append("lib").toOSString())).exists() || (new File(wsasHome.append("repository").append("lib").toOSString())).list().length<5){
			...
			runner.setBuildFileLocation(wsasHome.append("bin").append("build.xml").toOSString());
