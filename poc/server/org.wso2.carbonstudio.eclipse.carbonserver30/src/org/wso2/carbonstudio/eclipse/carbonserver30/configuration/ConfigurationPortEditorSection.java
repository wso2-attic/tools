	...
	protected void changePortNumber(String name, int port) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (sp.getName().equals(name)) {
				...
				//change - 21/04/2011
//				CarbonServer30Utils.setServerConfigMapValue(server.getOriginal(),sp.getName(),port+"");
				CarbonServer30Utils.setServerConfigMapValue(server.getOriginal(),sp.getId(),port+"");
				CarbonServer30Utils.updateTransportXML(server.getOriginal());
				CarbonServer30Utils.updateAxis2XML(server.getOriginal());
				/*if (i == selection) {
					selectPort();
				}*/
				return;
...
//			tomcatConfiguration.removePropertyChangeListener(listener);
	...
	protected void initialize() {
		...
		ports.removeAll();
		CarbonServer30Utils.isServerStartBrowserPopup(server.getOriginal());
