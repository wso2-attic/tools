...
public class QueryAction extends StaticSelectionCommandAction {
	...
	protected Command createActionCommand(EditingDomain editingDomain, Collection<?> collection) {
		...
		Iterator<ConfigurationProperty> iterator = config.getConfigurationProperties().iterator();
			...
			if (collection.size() == 1 && newChildDescriptors != null) {
				...
				for (Object descriptor : newChildDescriptors) {
					...
					if (childObj instanceof Query) {
						...
						query.setDataSourceId(config.getId());

						// Add a query element to the tree
						querycmd = CreateChildCommand.create(editingDomain,
								((DataSourceConfiguration) owner).eContainer(), descriptor,
								collection);
						...
						if (isRDBMSQuery || isJNDIQuery || isCarbonQuery) {
							...
							QueryPropertyList queryPropertyList = DsFactory.eINSTANCE
									.createQueryPropertyList();
							CommandParameter param2 = new CommandParameter(query,
									DsPackage.Literals.QUERY__PROPERTIES_LIST, queryPropertyList);
							...
							while (i.hasNext()) {
								...
								CommandParameter param3 = new CommandParameter(queryPropertyList,
										DsPackage.Literals.QUERY_PROPERTY_LIST__PROPERTIES,
										queryProperty);
								Command cmd3 = CreateChildCommand.create(editingDomain,
										queryPropertyList, param3, collection);
						...
						if (isSpreadSheetQuery) {
							...
							CommandParameter param2 = new CommandParameter(query,
									DsPackage.Literals.QUERY__EXCEL_QUERIES, excelQuery);
							Command cmd2 = CreateChildCommand.create(editingDomain, query, param2,
									collection);
						...
						if (isGSpreadQuery) {
							...
							gspreadQuery.setMaxRowCount(new BigInteger("-1"));

							CommandParameter param2 = new CommandParameter(query,
									DsPackage.Literals.QUERY__GSPREAD_QUERIES, gspreadQuery);
							Command cmd2 = CreateChildCommand.create(editingDomain, query, param2,
									collection);
