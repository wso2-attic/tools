...
public class CustomPropertyEditorFactory {
	...
	interface CustomPropertyEditorBuilder {
		CellEditor build(Composite composite, Object propertyContainer, IItemPropertyDescriptor propertyDescriptor);
	}	
	...
	static {
		...
		customPropertyEditorBuildersMap.put(NamespacedProperty.class, new CustomPropertyEditorBuilder() {
			public CellEditor build(Composite composite, Object propertyContainer,
					IItemPropertyDescriptor propertyDescriptor) {
				PropertyValueWrapper wrapper = (PropertyValueWrapper) propertyDescriptor.getPropertyValue(propertyContainer);
				NamespacedProperty namespacedProperty = (NamespacedProperty) wrapper.getEditableValue(propertyContainer);
				if (null != namespacedProperty) {
					return new NamespacedPropertyEditor(composite, namespacedProperty, propertyContainer, propertyDescriptor);
				}								
				return null;
			}
		});
		
		customPropertyEditorBuildersMap.put(RegistryKeyProperty.class, new CustomPropertyEditorBuilder() {
			public CellEditor build(Composite composite, Object propertyContainer,
					IItemPropertyDescriptor propertyDescriptor) {
				PropertyValueWrapper wrapper = (PropertyValueWrapper) propertyDescriptor.getPropertyValue(propertyContainer);
				RegistryKeyProperty registryProperty = (RegistryKeyProperty) wrapper.getEditableValue(propertyContainer);
				if (null != registryProperty) {
					return new RegistryKeyPropertyEditor(composite, registryProperty, propertyContainer, propertyDescriptor);
				}								
				return null;
			}
		});		
	 ...
	 */
	public static CellEditor createCustomPropertyEditor(Composite composite, Object propertyContainer, IItemPropertyDescriptor propertyDescriptor) {
		...
		if (feature instanceof EStructuralFeature) {
			...
			CustomPropertyEditorBuilder builder = customPropertyEditorBuildersMap.get(instanceClass);
			if (null != builder) {
