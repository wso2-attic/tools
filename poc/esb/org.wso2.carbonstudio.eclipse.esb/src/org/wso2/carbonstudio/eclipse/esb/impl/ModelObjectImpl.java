 ...
 * <ul>
 *   <li>{@link org.wso2.carbonstudio.eclipse.esb.impl.ModelObjectImpl#getObjectState <em>Object State</em>}</li>
 *   <li>{@link org.wso2.carbonstudio.eclipse.esb.impl.ModelObjectImpl#getSourceText <em>Source Text</em>}</li>
 *   <li>{@link org.wso2.carbonstudio.eclipse.esb.impl.ModelObjectImpl#getDefaultNamespace <em>Default Namespace</em>}</li>
 *   <li>{@link org.wso2.carbonstudio.eclipse.esb.impl.ModelObjectImpl#getAdditionalNamespaces <em>Additional Namespaces</em>}</li>
 * </ul>
 * </p>
 *
 * @generated
...
public abstract class ModelObjectImpl extends EObjectImpl implements ModelObject {
     ...
     * 
     * @param <T> {@link ModelObject} sub type.
     */
	 ...
	 */
	private MediatorFactory mediatorFactory = MediatorFactory.eINSTANCE;
		...
		if (getObjectState().equals(ModelObjectState.NORMAL)) {
			...
			if (null != self) {
				for (Namespace additionalNs : getAdditionalNamespaces()) {
					String nsAttributeName = String.format("xmlns:%s", additionalNs.getPrefix());
					
					// Do not override.
					if (!self.hasAttribute(nsAttributeName)) {
						self.setAttribute(nsAttributeName, additionalNs.getUri());
					}
				}
			}
			return self;
			...
			Comment comment = parent.getOwnerDocument().createComment(getSourceText());
	 ...
	 */
	protected MediatorFactory getMediatorFactory() {
	...
	protected <T extends ModelObject> void loadObject(Element parent, String childName, Class<T> clazz,
		...
		if (null != childElem) {
			...
			throw new Exception(String.format("The element [%s] is missing the required child element [%s].", parent
					.getLocalName(), childName));
		}
	}
	 ...
	 */
	protected <T extends ModelObject> void loadObjects(Element parent, String childName, Class<T> clazz,
			ObjectHandler<T> handler) throws Exception {
	 ...
	 *             if at least one object cannot be loaded.
	 */
	protected <T extends ModelObject> void loadObjects(Element parent, Class<T> clazz, ObjectHandler<T> handler)
			throws Exception {
	...
	/**
	 * Creates a child under the parent element with default namespace configuration.
	 * 
	...
	protected Element createChildElement(Element parent, String localName) {
		...
		if (null != defaultNs && !StringUtils.isBlank(defaultNs.getUri())) {			
			...
			if (!StringUtils.isBlank(defaultNs.getPrefix())) {
				qualifiedName = String.format("%s:%s", defaultNs.getPrefix(), localName);
			}
		} else if (USE_SYNAPSE_NAMESPACE_PREFIX) {
			qualifiedName = String.format("%s:%s", DEFAULT_SYNAPSE_NAMESPACE_PREFIX, localName);
		}
		
		Element child = parent.getOwnerDocument().createElementNS(namespaceUri, qualifiedName);
	...
	/**
	 * Creates a child under the parent element with the given namespace configuration.
	 * 
	 * @param parent parent {@link Element}.
	 * @param childNsURI namespace uri of the child element.
	 * @param childNsPrefix namespace prefix.
	 * @param childLocalName localname of the child element.
	 * @return newly created child element.
	 */
	protected Element createChildElement(Element parent, String childNsURI, String childNsPrefix, String childLocalName) {
		String qualifiedName = String.format("%s:%s", childNsPrefix, childLocalName);
		Element child = parent.getOwnerDocument().createElementNS(childNsURI, qualifiedName);
		parent.appendChild(child);
		return child;
	}
	...
	/**
	 * Utility method for getting a nested {@link CDATASection} directly under the given parent {@link Element}.
	 * 
	 * @param parent parent {@link Element}.
	 * @return {@link CDATASection} nested under the given parent {@link Element} or null.
	 */
	...
	/**
	 * Search within the parent element for all child elements bearing the specified local name.
	 * 
	 * @param parent parent {@link Element}.
	 * @param localName local name of child elements.
	...
	/**
	 * Extracts any namespace definitions ("xmlns:prefix=uri") declared on the element.
	 * 
	 * @param element {@link Element} to be examined.
	...
	/**
	 * Utility method for concatenating an string array with a provided separator.
	 * 
	 * @param strArray array of strings to be concatenated.
