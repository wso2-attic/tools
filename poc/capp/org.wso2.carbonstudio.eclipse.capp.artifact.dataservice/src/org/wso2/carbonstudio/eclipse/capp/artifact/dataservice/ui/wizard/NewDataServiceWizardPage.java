...
public class NewDataServiceWizardPage extends WizardPage {
	private static ICarbonStudioLog log=Logger.getLog(Activator.PLUGIN_ID);

	private Text fileText;
	private Button optNewDataService;
	...
	public NewDataServiceWizardPage(WizardNewFileCreationPage wizardNewFileCreationPage, IProject p) {
		super("wizardPage");
		setDescription("Select Data Service");
	...
	public void createControl(Composite parent) {
		...
		lblFrom.setText("Create Data Service artifact from:");
        ...
        optNewDataService = new Button(group,SWT.RADIO);
        optNewDataService.setText("New Data Service");
        gd = new GridData();
        gd.horizontalSpan=2;
        optNewDataService.setLayoutData(gd); 
        
		optNewDataService.addSelectionListener((new SelectionListener() {
        ...
        importFromFS.setText("Import data service from file system");
        gd = new GridData();
        gd.horizontalSpan=3;
...
//					if(filePathText.getText() == null || filePathText.getText().equals("")){
//						updateStatus("Specify a Data Service file from File System"); 
//					}else{
//						setFileName(ArtifactFileUtils.getFileNameWithoutExtension(filePathText.getText()));
//						updateStatus(null);
//					}
...
//		}
		...
		importFromRegistry.setText("Import data service from registry");
        gd = new GridData(GridData.FILL_HORIZONTAL);
//        gd.horizontalIndent = 10;
...
//					}
	...
	public void checkoutRegResource() throws Exception{
		...
		if(resourceName != null){
			...
			dsFile = new File(tmpFolder, info[info.length-1] );
			IRegistryData selectRegistryPathData = getSelectRegistryPathData();
			if (selectRegistryPathData==null){
				throw new Exception("Registry location to import the data service was not specified");
			}
			CAppEnvironment.getRegistryHandler().importRegistryResourceToFileSystem(selectRegistryPathData, dsFile);
			setFilePath(dsFile.getPath());
		}

	}
	
	public void popUPRegistryBrowser() throws Exception{
		...
		String title = "Browse registry for the data service file";
		String message = "Select the path of the data service file";
	...
	private void initialize() {
		optNewDataService.setSelection(true);
		fileText.setEnabled(true);
		lblFromNew.setEnabled(true);
...
//		fileText.setText("newproxyservice");
	...
	public boolean handleFilePathText() {
		...
		if (!file.exists()) {
			...
			if (ext != null) {
				if (!ext.equals(".dbs")) {
					msg = "Data service file should be with the extension of .dbs. Please specify the correct data service file ";
					isPathValid = false;
				} else {
					isPathValid = true;
				}
	...
	private String getSavePath() {
		...
		fld.setFilterExtensions(new String[]{"*.dbs"});
		boolean done = false;
		while (!done) {
		...
		if(fileName!= null && !fileName.equals("")){
			setFileName(ArtifactFileUtils.getFileNameWithoutExtension(fileName));
		}
