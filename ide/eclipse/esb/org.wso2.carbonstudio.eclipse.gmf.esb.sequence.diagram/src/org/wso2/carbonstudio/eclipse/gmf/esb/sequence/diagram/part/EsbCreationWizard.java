...
public class EsbCreationWizard extends Wizard implements INewWizard {
	...
	public void addPages() {
		diagramModelFilePage = new EsbCreationWizardPage(
				"DiagramModelFile", getSelection(), "sequence_diagram"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		domainModelFilePage = new EsbCreationWizardPage(
				"DomainModelFile", getSelection(), "sequence") { //$NON-NLS-1$ //$NON-NLS-2$

			public void setVisible(boolean visible) {
				if (visible) {
					String fileName = diagramModelFilePage.getFileName();
					fileName = fileName.substring(0, fileName.length()
							- ".sequence_diagram".length()); //$NON-NLS-1$
					setFileName(EsbDiagramEditorUtil.getUniqueFileName(
							getContainerFullPath(), fileName, "sequence")); //$NON-NLS-1$
				}
