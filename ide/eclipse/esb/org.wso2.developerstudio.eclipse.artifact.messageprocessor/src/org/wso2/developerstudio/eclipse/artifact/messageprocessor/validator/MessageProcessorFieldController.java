...
public class MessageProcessorFieldController extends AbstractFieldController{
	...
	public void validate(String modelProperty, Object value, ProjectDataModel model) throws FieldValidationException {
		...
		boolean isScheduledMessage = processModel.getMessageProcessorType().equals("Scheduled Message Forwarding Processor");
		boolean isSamplingProcessor = processModel.getMessageProcessorType().equals("Message Sampling Processor");
		boolean isCustomProcessor = processModel.getMessageProcessorType().equals("Custom Message Processor");
		...
		if(modelProperty.equals("processor.name")){
			...
			if(processorModel.getAvailableProcessorlist() != null && processorModel.getAvailableProcessorlist().size()>0){
				if(processorModel.getSelectedProcessorList() == null || processorModel.getSelectedProcessorList().size() == 0){
	...
	public boolean isVisibleField(String modelProperty, ProjectDataModel model) {
		...
		if(modelProperty.startsWith("FS_processor.")){
			if(((MessageProcessorModel)model).getMessageProcessorType().equals("Scheduled Message Forwarding Processor") || ((MessageProcessorModel)model).getMessageProcessorType().equals("Message Sampling Processor")){
				visibleField = true;
			}else{
				visibleField = false;
			} 
		}else if(modelProperty.startsWith("Forwarding_processor.")){
			visibleField = ((MessageProcessorModel)model).getMessageProcessorType().equals("Scheduled Message Forwarding Processor");
		}else if(modelProperty.startsWith("sampling_processor.")){
			visibleField = ((MessageProcessorModel)model).getMessageProcessorType().equals("Message Sampling Processor");
		}else if(modelProperty.startsWith("custom_processor.")){
			visibleField = ((MessageProcessorModel)model).getMessageProcessorType().equals("Custom Message Processor");
		}else if(modelProperty.equals("available.processors")){
			List<OMElement> availableProcessors = ((MessageProcessorModel) model).getAvailableProcessorlist();
			visibleField = (availableProcessors != null && availableProcessors.size() > 0);
		}
