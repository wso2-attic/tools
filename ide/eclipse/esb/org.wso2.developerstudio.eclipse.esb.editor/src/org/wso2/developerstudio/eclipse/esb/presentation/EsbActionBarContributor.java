...
/**
 * This is the action bar contributor for the Esb model editor.
 * <!-- begin-user-doc -->
 * <!-- end-user-doc -->
 * @generated
 */
public class EsbActionBarContributor
	...
	public void contributeToMenu(IMenuManager menuManager) {
        ...
        IMenuManager submenuManager = new MenuManager(EsbEditorPlugin.INSTANCE.getString("_UI_EsbEditor_menu"), "org.wso2.developerstudio.eclipse.esbMenuID");
        ...
        //
        createChildMenuManager = new MenuManager(EsbEditorPlugin.INSTANCE.getString("_UI_CreateChild_menu_item"));
        submenuManager.insertBefore("additions", createChildMenuManager);

        // Prepare for CreateSibling item addition or removal.
        //
        createSiblingMenuManager = new MenuManager(EsbEditorPlugin.INSTANCE.getString("_UI_CreateSibling_menu_item"));
	...
	protected Collection<IAction> generateCreateChildActions(Collection<?> descriptors, ISelection selection) {
        ...
        if (descriptors != null) {
            for (Object descriptor : descriptors) {            	
            	IAction action = new CreateChildAction(activeEditorPart, selection, descriptor);     
            	fixActionText(action, descriptor);
                actions.add(action);
            }
        }
        return actions;
	...
	protected Collection<IAction> generateCreateSiblingActions(Collection<?> descriptors, ISelection selection) {
        ...
        if (descriptors != null) {
            for (Object descriptor : descriptors) {
            	IAction action = new CreateSiblingAction(activeEditorPart, selection, descriptor);
            	fixActionText(action, descriptor);
                actions.add(action);
            }
        }
	...
	/**
	 * Utility method for adjusting an {@link IAction}'s text property to support sub-menus.
	 * 
	 * @param action {@link IAction} instance.
	 * @param descriptor action descriptor.
	 */
	private void fixActionText(IAction action, Object descriptor) {
		if (descriptor instanceof CommandParameter) {
    		CommandParameter param = (CommandParameter) descriptor;
    		Object value = param.getValue();
    		if (value instanceof EObject) {
    			String resourceKey = String.format("_UI_%s_category", ((EObject) value).eClass().getName());
    			try {
    				String category = EsbEditorPlugin.INSTANCE.getString(resourceKey);
    				action.setText(String.format("%s|%s", category, action.getText()));
    			} catch (Exception ex) {
    				// Ignore.
    			}
    		}
    ...
    protected Map<String, Collection<IAction>> extractSubmenuActions(Collection<IAction> createActions) {
    	// Use a TreeMap for sorting menu entries.
        Map<String, Collection<IAction>> createSubmenuActions = new TreeMap<String, Collection<IAction>>(new Comparator<Object>() {
	...
	public void menuAboutToShow(IMenuManager menuManager) {
        ...
        submenuManager = new MenuManager(EsbEditorPlugin.INSTANCE.getString("_UI_CreateChild_menu_item"));
        populateManager(submenuManager, createChildSubmenuActions, null);
        populateManager(submenuManager, createChildActions, null);
        menuManager.insertBefore("edit", submenuManager);

        submenuManager = new MenuManager(EsbEditorPlugin.INSTANCE.getString("_UI_CreateSibling_menu_item"));
