...
public class ConfigureMessageStoreParametersDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
				...
				| SWT.FULL_SELECTION | SWT.HIDE_SELECTION);
		{
			...
			// Populate template.
			for (MessageStoreParameter parameter : store.getParameters()) {
	...
	private TableItem bindParameter(MessageStoreParameter parameter) {
	...
	private void unbindParameter(int itemIndex) {
		...
		MessageStoreParameter Parameter = (MessageStoreParameter) item.getData();
		if (null != Parameter.eContainer()) {
			RemoveCommand removeCmd = new RemoveCommand(editingDomain,
					store, EsbPackage.Literals.MESSAGE_STORE__PARAMETERS,
					Parameter);
	...
	protected void okPressed() {
		...
		for (TableItem item : parametersTable.getItems()) {
			...
			if (null == parameter.eContainer()) {
				...
				AddCommand addCmd = new AddCommand(editingDomain, store,
						EsbPackage.Literals.MESSAGE_STORE__PARAMETERS, parameter);
				...
				if (!parameter.getParameterName().equals(item.getText(0))) {
					SetCommand setCmd = new SetCommand(
							editingDomain,
							parameter,
							EsbPackage.Literals.MESSAGE_STORE_PARAMETER__PARAMETER_NAME,
							item.getText(0));
				...
				if (!parameter.getParameterValue().equals(item.getText(1))) {
					SetCommand setCmd = new SetCommand(
							editingDomain,
							parameter,
							EsbPackage.Literals.MESSAGE_STORE_PARAMETER__PARAMETER_VALUE,
							item.getText(1));
					getResultCommand().append(setCmd);
				}
			}
