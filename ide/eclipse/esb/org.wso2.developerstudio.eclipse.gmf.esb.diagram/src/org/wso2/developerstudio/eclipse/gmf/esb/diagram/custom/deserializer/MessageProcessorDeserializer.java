...
/**
 * Deserializes a message-processor configuration to a graphical message processor object
 */
		...
		if(processor instanceof DummyMessageProcessor) {
			...
			if(StringUtils.isNotBlank(dummyMessageProcessor.getClassName())) {
				if(dummyMessageProcessor.getClassName().equals(scheduledMessageForwardingProcessor)) {
					...
					executeSetValueCommand(MESSAGE_PROCESSOR__PROCESSOR_TYPE, MessageProcessorType.SCHEDULED_MSG_FORWARDING);
					...
					if(parameters.containsKey("max.delivery.attempts")) {
						...
						if(value != null && StringUtils.isNumeric(value.toString())) {
							executeSetValueCommand(MESSAGE_PROCESSOR__MAX_DELIVERY_ATTEMPTS, new Integer(value.toString()));
						}
					}
					...
					if(parameters.containsKey("pinnedServers")) {
						...
						if(StringUtils.isNotBlank(value.toString())) {
							executeSetValueCommand(MESSAGE_PROCESSOR__PINNED_SERVERS, value.toString());
						}
					}
					...
					executeSetValueCommand(MESSAGE_PROCESSOR__MESSAGE_PROCESSOR_PROVIDER, dummyMessageProcessor.getClassName());
					
					for(Entry<String, Object> parameter : dummyMessageProcessor.getParameters().entrySet()) {
						MessageProcessorParameter processorParameter = EsbFactory.eINSTANCE.createMessageProcessorParameter();
