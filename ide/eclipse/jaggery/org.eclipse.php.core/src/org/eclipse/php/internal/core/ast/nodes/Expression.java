...
public abstract class Expression extends ASTNode {
	...
	public boolean isStaticScalar() {
		...
		if (type != ASTNode.SCALAR && type != ASTNode.UNARY_OPERATION
				&& type != ARRAY_CREATION) {
			return false;
		}
		...
		if (parentType == ASTNode.FORMAL_PARAMETER
				|| parentType == ASTNode.DECLARE_STATEMENT
		...
		if (parentType == ASTNode.ASSIGNMENT) {
			...
			if (grandpa.getType() == ASTNode.VAR_STATEMENT) {
				return true;
			}
			return false;
		}
		...
		if (parentType == ASTNode.SCALAR
				|| parentType == ASTNode.UNARY_OPERATION
				|| parentType == ARRAY_CREATION) {
	...
	public Object resolveConstantExpressionValue() {
		return this.ast.getBindingResolver().resolveConstantExpressionValue(
				this);
	}
}
