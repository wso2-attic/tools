...
import org.wso2.maven.qos.utils.QOSProjectMavenUtils;
 ...
 * 
 */
public class ServiceMetaPOMGenMojo extends AbstractPOMGenMojo {
	...
	private static final String ARTIFACT_TYPE="service/meta";
	
	private List<QOSArtifact> artifacts;
	
	private Map<Artifact, QOSArtifact> artifactToRegArtifactMap;
	
	private List<QOSArtifact> retrieveArtifacts() {
		return QOSProjectMavenUtils.retrieveArtifacts(getArtifactLocation());
	}
	
	public void execute() throws MojoExecutionException, MojoFailureException {
		//Retrieving all the existing ESB Artifacts for the given Maven project 
				List<QOSArtifact> artifacts = retrieveArtifacts();
				...
				for (QOSArtifact esbArtifact : artifacts) {
			...
			protected void addPlugins(MavenProject artifactMavenProject, Artifact artifact) {
				Plugin plugin = CAppMavenUtils.createPluginEntry(artifactMavenProject,"org.wso2.maven","wso2-qos-project-plugin",WSO2MavenPluginConstantants.WSO2_QOS_VERSION,true);
