...
public class WSASWebservicesServerCommand extends AbstractDataModelOperation {
	...
	throws ExecutionException {
		...
		IPath webInfPath = J2EEUtils.getWebInfPath(model.getWebProject());
		String webContainerDirString = ResourcesPlugin.getWorkspace().getRoot().getFolder(webInfPath).getLocation().toOSString();
		...
		if (model.getServerStatus()){
			if (scenario == WebServiceScenario.BOTTOMUP){
				...
				String repositoryString = webContainerDirString+File.separator + WSASCreationUIMessages.DIR_SERVICES; 
