...
//		XSModel model = xmlSchemaLoader.loadURI("file:/home/saminda/workspaces/dataservice-workspace/ABC/schema/DataService.xsd");
...
//		XSTypeDefinition typeDefinition = model.getTypeDefinition("data", null);
//		printXS(model.getNamespaceItems(),-1);
//			XSNamedMap components = model.getComponents((short) XSConstants.ELEMENT_DECLARATION);
	...
	private static void printDocElement(DataserviceTagElement ele, int level) {
		...
		if (ele.getAttributes().size()>0){
			...
			for (DataserviceAttributeElement attr : ele.getAttributes()) {
				...
				if (attr.isOptional()) attrList+="(optional)";
			}
