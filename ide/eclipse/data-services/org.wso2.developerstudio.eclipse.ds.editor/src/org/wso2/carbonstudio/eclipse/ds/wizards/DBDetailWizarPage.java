...
public class DBDetailWizarPage extends WizardPage implements Listener{
	...
	public void createControl(Composite parent) {
		...
		tableViewer = new TableViewer(client, SWT.MULTI
				| SWT.H_SCROLL | SWT.V_SCROLL | SWT.FULL_SELECTION | SWT.BORDER | SWT.CHECK);
	...
	private void createTableViewerColumns(Composite parent, TableViewer viewer) {
		...
		TableViewerColumn col1 = createTableViewerColumn(viewer, titles[0],
				bounds[0]);
	...
	public IWizardPage getNextPage() {
		...
		if (this.selectedTables != null && selectedTables.size() != 0) {
			//((DataServiceCreationWizard) getWizard()).serviceGenModePage.setConnection(this.);
			((DataServiceCreationWizard) getWizard()).serviceGenModePage
					.setSelectedTableNames(this.selectedTables);
