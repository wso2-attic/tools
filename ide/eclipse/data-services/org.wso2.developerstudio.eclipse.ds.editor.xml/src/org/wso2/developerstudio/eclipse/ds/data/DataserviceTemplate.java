...
public class DataserviceTemplate {
	...
	static {
		...
		XSModel model = xmlSchemaLoader.loadURI(DataserviceUtils.getDataserviceSchemaPath().toURI().toString());
	...
	private static void printXS(XSNamespaceItem obj, int level) {
		...
		for (int i = 0; i < components.getLength(); i++) {
			...
			DataserviceTagElement dataserviceTagElement = new DataserviceTagElement(null,item.getName());
...
//				System.out.print(" [");
				for (int j = 0; j < attributeUses.getLength(); j++) {
					...
					parentElement.getAttributes().add(new DataserviceAttributeElement(name,attr.getRequired()));
//					if (j==0){
...
//				System.out.println();
			...
			if (typeDef.getParticle()!=null && typeDef.getParticle().getTerm() instanceof XSModelGroupImpl){
				...
				for (int j = 0; j < grp.getParticles().getLength(); j++) {
					if (grp.getParticles().item(j) instanceof XSParticleDecl){
						...
						if (part.fValue instanceof XSElementDecl){
							DataserviceTagElement docElement=new DataserviceTagElement(parentElement,((XSElementDecl)part.fValue).getName());
