...
public class DataserviceQuickAssistantProcessor implements IQuickAssistProcessor {
	...
	class SchemaCompletionProposal implements ICompletionProposal{
	  ...
	  public void apply(IDocument doc) {
	    ...
	    int offset = structuredDocumentRegion.getStartOffset()+DataserviceQuickAssistantProcessor.PROJECT_NODE.length()+1;
	    if(offset <= 0){
	      return;
	    }
	    ...
	    try {
	      ...
	      Display.getDefault().asyncExec(new Runnable(){
	        public void run(){
	          ...
	          Activator.getDefault().getWorkbench().getActiveWorkbenchWindow().getActivePage().saveEditor(activeEditor, false);
	        }
	      });
	    } catch(Exception e){
	    	log.error(e);
