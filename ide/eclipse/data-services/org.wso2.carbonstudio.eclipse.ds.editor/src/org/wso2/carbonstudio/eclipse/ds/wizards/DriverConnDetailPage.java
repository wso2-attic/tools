package org.wso2.carbonstudio.eclipse.ds.wizards;
...
import org.wso2.carbonstudio.eclipse.ds.wizards.util.DBUriConstants;
import org.wso2.carbonstudio.eclipse.ds.wizards.util.DbConnManager;
import org.wso2.carbonstudio.eclipse.ds.wizards.util.DriverShim;
import org.wso2.carbonstudio.eclipse.ds.wizards.util.ValidateUtil;
...
public class DriverConnDetailPage extends WizardPage implements Listener{
			...
			public void widgetSelected(SelectionEvent event) {
				...
				if (ValidateUtil.isTextNonEmpty(drivePathTxt)) {
					if (ValidateUtil.isTextNonEmpty(userNameTxt)
						...
						try {
							Connection conn= testConnection(dbDetails.get(DBUriConstants.DB_URL),
									userNameTxt.getText(), passwdTxt.getText(),
	...
	private void generateDbUrl() {
		...
		if (dataBaseType.equals(DBUriConstants.MYSQL_ID)) {
			 ...
			 */
			url = DBUriConstants.REGEX_RDBMS + DBUriConstants.REGEX_COLON
			...
			if (dbDetails.get(DBUriConstants.ORC_TYPE) != null) {
				...
				if (orcUrlType.equals(DBUriConstants.ORC_TYPE_ONE)) {
					 ...
					 */
					url = DBUriConstants.REGEX_RDBMS
							+ DBUriConstants.REGEX_COLON
							+ DBUriConstants.ORACLE_ID
							+ DBUriConstants.REGEX_COLON
							+ dbDriveTxt.getText().trim()
							+ DBUriConstants.REGEX_COLON
							+ DBUriConstants.REGEX_AT
							+ DBUriConstants.REGEX_DSLASH
							+ DBUriConstants.REGEX_COLON
							+ hostTxt.getText().trim()
							+ DBUriConstants.REGEX_COLON
							+ portTxt.getText().trim()
							+ DBUriConstants.REGEX_SLASH
			...
			url = DBUriConstants.REGEX_RDBMS + DBUriConstants.REGEX_COLON
					+ DBUriConstants.DB2_ID + DBUriConstants.REGEX_COLON
					+ dbNameTxt.getText().trim();
			...
			url = DBUriConstants.REGEX_RDBMS + DBUriConstants.REGEX_COLON
					+ DBUriConstants.HSQL_ID + DBUriConstants.REGEX_COLON
					+ hostTxt.getText().trim();
			...
			url = DBUriConstants.REGEX_RDBMS + DBUriConstants.REGEX_COLON
					...
					+ DBUriConstants.REGEX_SLASH + dbNameTxt.getText().trim()
					+ DBUriConstants.REGEX_COLON
					+ DBUriConstants.INFORMIXSERVER + DBUriConstants.REGEX_EQL
					+ dbServerTxt.getText().trim();
			...
			url = DBUriConstants.REGEX_RDBMS + DBUriConstants.REGEX_COLON
					+ DBUriConstants.H2_ID + DBUriConstants.REGEX_COLON
					+ DBUriConstants.H2_TCP + DBUriConstants.REGEX_COLON
					+ hostTxt.getText().trim() + DBUriConstants.REGEX_COLON
					+ portTxt.getText().trim() + DBUriConstants.REGEX_SLASH
					+ dbNameTxt.getText().trim();
		}
	...
	public Connection testConnection(String dbURL, String dbUsername,
			String dbPassword, String driverURL, String driverClass) throws MalformedURLException, InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException{
		...
		try {
			...
			if (connection != null) {
				//isConnAlive = true;
				MessageDialog.openInformation(Display.getCurrent()
						.getActiveShell(), "Success!", "Ping Succeeded!");
			} else {
				
				MessageDialog.openInformation(Display.getCurrent()
						.getActiveShell(), "Error!", "Ping Failed!");
			}
