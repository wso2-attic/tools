...
package org.wso2.carbonstudio.eclipse.carbonserver32.configuration;
...
import org.wso2.carbonstudio.eclipse.carbonserver32.Activator;
	...
	public void createSection(Composite parent) {
		...
		startWSASCheck.addSelectionListener(new SelectionAdapter(){
			public void widgetSelected(SelectionEvent arg0) {
				CarbonServer32Utils.setServerStartBrowserPopup(server.getOriginal(),startWSASCheck.getSelection());
			}
		});
		...
		hotupdateCheck.addSelectionListener(new SelectionAdapter(){
			public void widgetSelected(SelectionEvent arg0) {
				CarbonServer32Utils.setServerHotUpdate(server.getOriginal(), hotupdateCheck.getSelection());
			}
		...
		osgiConsoleCheck.addSelectionListener(new SelectionAdapter(){
			public void widgetSelected(SelectionEvent arg0) {
				CarbonServer32Utils.setServerStartWithOSGiConsole(server.getOriginal(),osgiConsoleCheck.getSelection());
			}
		});
...
//		axis2FileMonitor.stopMonitoring();
	...
	protected void initialize() {
		Display.getDefault().asyncExec(new Runnable(){
            public void run() {
            	while(true){
            		Boolean serverConfigMapValue = CarbonServer32Utils.isServerStartBrowserPopup(server.getOriginal());
            		...
            		boolean enableAxis2Hotupdate=CarbonServer32Utils.isServerHotUpdate(server.getOriginal());
            		boolean enableOSGIConsole=CarbonServer32Utils.isServerStartWithOSGiConsole(server.getOriginal());
