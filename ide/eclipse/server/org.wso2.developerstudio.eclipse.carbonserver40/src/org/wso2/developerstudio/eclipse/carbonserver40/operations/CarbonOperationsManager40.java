...
//					break;
				...
				case ICarbonOperationManager.OPERATION_GET_SERVER_CREDENTIALS:
					...
					if (server!=null){
						serverCredentials=CarbonServer40Utils.getServerCredentials(server);
					}
				...
				case ICarbonOperationManager.OPERATION_GET_SERVER_AUTHENTICATED_COOKIE:
					...
					if (server!=null){
						if (operation.containsKey(ICarbonOperationManager.PARAMETER_SERVER_PORT)){
							...
							cookie=CarbonServer40Utils.getServerCookie(server,serverPort);
						}
					}
					result=cookie;
					break;
...
//		String[] paths=new String[]{"repository","deployment","server"};
		String serverLocalRepoLocation=CarbonServer40Utils.getRepositoryPathFromLocalWorkspaceRepo(serverLocalWorkspacePath);
//		paths=new String[]{"repository","conf"};
		String serverLocalConfLocation=CarbonServer40Utils.getConfPathFromLocalWorkspaceRepo(serverLocalWorkspacePath);
...
//		CarbonServer31Utils.updateAndSaveCarbonXml(FileUtils.addNodesToPath(serverLocalConfLocation,new String[]{"carbon.xml"}), serverLocalRepoLocation,server);
		...
		CarbonServer40Utils.isServerHotUpdate(server);
