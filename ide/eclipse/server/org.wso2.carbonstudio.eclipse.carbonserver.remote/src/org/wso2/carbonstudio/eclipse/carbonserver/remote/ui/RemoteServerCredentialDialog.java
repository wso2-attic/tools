	...
	public Control createDialogArea(Composite parent) {
		...
		btnNewButton.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				try {
					RemoteCarbonServerUtils.validateCredentials(remoteCServer.getServerURL().toString(), 
																getUsername(), 
																getPwd());
					MessageDialog.openInformation(getShell(), "Validation", "Credentials have been successfully validated");
				} catch (Exception e) {
					MessageDialog.openError(getShell(), "Validation Failed", 
