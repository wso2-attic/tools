...
package org.wso2.carbonstudio.eclipse.carbonserver30.configuration;
...
import org.wso2.carbonstudio.eclipse.carbonserver30.Activator;
	...
	public void createSection(Composite parent) {
		...
		startWSASCheck.addSelectionListener(new SelectionAdapter(){
			public void widgetSelected(SelectionEvent arg0) {
				CarbonServer30Utils.setServerStartBrowserPopup(server.getOriginal(),startWSASCheck.getSelection());
			}
		});
		...
		hotupdateCheck.addSelectionListener(new SelectionAdapter(){
			public void widgetSelected(SelectionEvent arg0) {
				CarbonServer30Utils.setServerHotUpdate(server.getOriginal(), hotupdateCheck.getSelection());
			}
		...
		osgiConsoleCheck.addSelectionListener(new SelectionAdapter(){
			public void widgetSelected(SelectionEvent arg0) {
				CarbonServer30Utils.setServerStartWithOSGiConsole(server.getOriginal(),osgiConsoleCheck.getSelection());
			}
		});
		
//		axis2FileMonitor = new FileMonitor(CarbonServer30Utils.getAxis2FilePath(server.getOriginal()));
...
//		if (axis2FileChanged){
//			axis2FileChanged=false;
//			if (hotupdateCheck!=null) hotupdateCheck.setSelection(CarbonServer30Utils.isHotUpdateEnabled(server.getOriginal()));
//		}
//		getShell().getDisplay().timerExec(axis2FileChangeCheckDelay, this);
//	} 
...
//		axis2FileMonitor.stopMonitoring();
	...
	protected void initialize() {
		Display.getDefault().asyncExec(new Runnable(){
            public void run() {
            	while(true){
            		Boolean serverConfigMapValue = CarbonServer30Utils.isServerStartBrowserPopup(server.getOriginal());
            		...
            		boolean enableAxis2Hotupdate=CarbonServer30Utils.isServerHotUpdate(server.getOriginal());
            		boolean enableOSGIConsole=CarbonServer30Utils.isServerStartWithOSGiConsole(server.getOriginal());
